...
if matches[1] == "pin" and is_owner(msg) or matches[1] == "سنجاق" and is_owner(msg) then
...
if matches[1] == "add" or matches[1] == "اتصال" then
return modadd(msg)
end
if matches[1] == "rem" or matches[1] == "قطع" then
return modrem(msg)
end
if matches[1] == "setowner" and is_admin(msg) or matches[1] == "مدیر" and is_admin(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
...
if matches[1] == "promote" and is_owner(msg) or matches[1] == "ارتقا" and is_owner(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
    tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
...
if matches[1] == "demote" and is_owner(msg) or matches[1] == "عزل" and is_owner(msg) then
if not matches[2] and tonumber(msg.reply_to_message_id_) ~= 0 then
 tdcli_function ({
      ID = "GetMessage",
      chat_id_ = msg.chat_id_,
...
if matches[2] == "flood" or matches[2] == "رگبار" then
return lock_flood(msg, data, target)
end
if matches[2] == "bots" or matches[2] == "رباتها" then
...
if matches[1] == "mute" and is_mod(msg) or matches[1] == "باصدا کردن" and is_mod(msg) then
local target = msg.chat_id_
...
if matches[2] == "contact" or matches[2] == "شماره" then
...
if matches[2] == "tgservice" or matches[2] == "ورود و خروج" then
...
if matches[1] == "unmute" and is_mod(msg) or matches[1] == "بیصدا کردن" and is_mod(msg)then
local target = msg.chat_id_
if matches[2] == "all" or matches[2] == "همه" then
...
if matches[1] == "whois" and matches[2] and is_mod(msg) or matches[1] == "کیه" and matches[2] and is_mod(msg)then
tdcli_function ({
    ID = "GetUser",
    user_id_ = matches[2],
  }, action_by_id, {chat_id=msg.chat_id_,user_id=matches[2],cmd="whois"})
  end
  if matches[1] == 'setflood' and is_mod(msg) or matches[1] == 'حساسیت' and is_mod(msg) then
    ...
    return "_Group_ *flood* _sensitivity has been set to :_ *[ "..matches[2].." ]*"
     ...
     if not lang then
       ...
       if not lang then
			...
			if matches[2] == 'welcome' or matches[2] == 'خوش آمد' then
   ...
   tdcli.changeChannelAbout(chat, "", dl_cb, nil)
		...
		if matches[1]:lower() == 'clean' and is_admin(msg) or matches[1]:lower() == 'پاک کن' and is_admin(msg) then
...
if matches[1] == "setname" and matches[2] and is_mod(msg) or matches[1] == "تنظیم اسم" and matches[2] and is_mod(msg)then
local gp_name = matches[2]
tdcli.changeChatTitle(chat, gp_name, dl_cb, nil)
end
  if matches[1] == "setabout" and matches[2] and is_mod(msg) or matches[1] == "ایجاد موضوع" and matches[2] and is_mod(msg)then
  ...
  if matches[1] == "about" and gp_type(chat) == "chat" or matches[1] == "موضوع" and gp_type(chat) == "chat" then
 if not data[tostring(chat)]['about'] then
     if not lang then
...
if matches[1] == "modlist" or matches[1] == "ناظران" then
return modlist(msg)
end
if matches[1] == "ownerlist" and is_owner(msg) or matches[1] == "مدیران" and is_owner(msg) then
...
if matches[1] == "help" and is_mod(msg)if matches[1] == "راهنما" and is_mod(msg) then
if not lang then
text = [[
🔰*Bot Commands:*🔰

در حال حاضر زبان ربات انگلیسی میباشد برای تغییر زبان دستور زیر را ارسال کنید
*!setlang fa* === تنظیم زبان فارسی

👑*!setowner* `[username|id|reply]` ==== مدیر
_Set Group Owner(Multi Owner)_

👑*!remowner* `[username|id|reply]` ==== حذف مدیر
 _Remove User From Owner List_

🤖*!promote* `[username|id|reply]` ==== ارتقا
_Promote User To Group Admin_

🤖*!demote* `[username|id|reply]` ==== عزل
_Demote User From Group Admins List_

🗣*!setflood* `[1-50]` ==== حساسیت
_Set Flooding Number_

🔇*!silent* `[username|id|reply]` ===== ساکت
_Silent User From Group_

🔊*!unsilent* `[username|id|reply]` ===== ور بزن
_Unsilent User From Group_

👽*!kick* `[username|id|reply]` ===== سیکتیر
_Kick User From Group_

👽*!ban* `[username|id|reply]` ==== مسدود
_Ban User From Group_

👽*!unban* `[username|id|reply]` ===== آزادی
...
قفل* `[لینک | @ | فارسی | ویرایش | فحش | سایت | رباتها | اسپم | رگبار | فونت | هایپرلینک]`
_If This Actions Lock, Bot Check Actions And Delete Them_

🔓*!unlock* `[link | tag | arabic | edit | fosh | webpage | bots | spam | flood | markdown | mention]`
بازکردن* `[لینک | @| فارسی | ویرایش| فحش | سایت | رباتها | اسپم | رگبار | فونت | هایپرلینک]`
_If This Actions Unlock, Bot Not Delete Them_

🔕*!mute* `[gifs | photo | tgservice | document | sticker | video | text | forward | location | audio | voice | contact | all]`
باصدا کردن* `[گیف | عکس | ورود خروج | فایل | استیکر | فیلم | چت | فروارد | موقعیت | اهنگ | ویس | شماره | همه]`
_If This Actions Lock, Bot Check Actions And Delete Them_

🔔*!unmute* `[gif | photo | tgservice | document | sticker | video | text | forward | inline | location | audio | voice | contact | all]`
بیصدا کردن* `[گیف | عکس | ورود و خروج | فایل | استیکر | فیلم | چت | فروارد | اینلاین | موقعیت | اهنگ | ویس | شماره | همه]`
_If This Actions Unlock, Bot Not Delete Them_

🔹*!set*`[rules | name | photo | link | about]`
 تنظیم*`[قوانین | اسم | عکس | لینک | ]`
 ****
 ایجاد موضوع ==== setabout
_Bot Set Them_

🔹*!clean* `[bans | mods | bots | rules | about | silentlist]`  
پاک کردن* `[مسدودین | ناظران | رباتها | قوانین | اطلاعات | خفه خونه]`
_Bot Clean Them_
...
if matches[1] == "قفلها" and is_mod(msg) or if matches[1] == "locks" and is_mod(msg)then
text2 = [[
...
if matches[1] == "ممنوع" and is_mod(msg) or  matches[1] == "mutes" and is_mod(msg) then
text3 = [[
🔕 لیست ممنوعیت ها 🔕


💬 ارسال گیف ممنوع🔊
🔇*!mute gif*===== بیصدا کردن گیف
🔊*!unmute gif*===== باصدا کردن گیف
〰〰〰〰〰
💬 ارسال عکس ممنوع🔊
🔇*!mute photo*🔊 ===== بیصدا کردن عکس
🔊*!unmute photo*🔊 ===== باصدا کردن عکس
〰〰〰〰〰
💬 ارسال فایل ممنوع🔊
🔇*!mute document*🔊 ===== بیصدا کردن فایل
🔊*!unmute document*🔊 ===== باصدا کردن  فایل
〰〰〰〰〰
💬 ارسال استیکر ممنوع🔊
🔇*!mute sticker*🔊 ===== بیصدا کردن استیکر 
🔊*!unmute sticker*🔊 ===== باصدا کردن استیکر
〰〰〰〰〰
💬 ارسال ویدیو ممنوع🔊
...
〰〰〰〰〰
💬 ارسال متن ممنوع🔊
🔇*!mute text*🔊 ===== بیصدا کردن چت
🔊*!unmute text*🔊 ===== باصدا کردن چت
〰〰〰〰〰
💬 ارسال فوروارد ممنوع🔊
🔇*!mute forward*🔊===== بیصدا کردن فروارد
🔊*!unmute forward*🔊 ====باصدا کردن فروارد
〰〰〰〰〰
💬 ارسال بازی به گروه🔊
🔇*!mute game*🔊 ===== بیصدا کردن بازی
🔊*!unmute game*🔊 ===== باصدا کردن بازی
〰〰〰〰〰
💬 ارسال مکان ممنوع🔊
🔇*!mute location*🔊 ====== باصدا کردن موقعیت
🔊*!unmute location*🔊 ===== باصدا کردن موقعیت
〰〰〰〰〰
💬 ارسال موزیک ممنوع🔊
🔇*!mute audio*🔊 ===== بیصدا کردن اهنگ
🔊*!unmute audio*🔊 ===== باصدا کردن اهنگ
〰〰〰〰〰
💬 ارسال فایل ضبط شده ممنوع🔊
🔇*!mute voice*🔊 ===== بیصدا کردن ویس
🔊*!unmute voice*🔊 ===== باصدا کردن ویس
〰〰〰〰〰
💬 ارسال اطلاعات تماس ممنوع🔊
🔇*!mute contact*🔊 ==== بیصدا کردن شماره
🔊*!unmute contact*🔊 ==== باصدا کردن شماره
〰〰〰〰〰
💬 اعلانات گروه ممنوع🔊
🔇*!mute tgservices*🔊 ===== بیصدا کردن ورود و خروج
🔊*!unmute tgservices*🔊 ======= باصدا کردن ورود و خروج
〰〰〰〰〰
💬 ارسال تبلیغات شیشه ای ممنوع🔊
🔇*!mute inline*🔊 ===== بیصدا کردن اینلاین
🔊*!unmute inline *🔊 ===== باصدا کردن اینلاین
〰〰〰〰〰
💬 همه چیز ممنوع🔊
🔇*!mute all*🔊 ====== بیصدا کردن همه
🔊*!unmute all*🔊 ====== باصدا کردن همه
〰〰〰〰〰
💬 میوت تایم دار🔊
...
if matches[1] == "مدیریت" and is_mod(msg) or matches[1] == "setting" and is_mod(msg)then
text4 = [[
...
〰〰〰〰〰
🚿 *!clean bans*====== پاک کن رباتها
💬  پاک کردن لیست مسدود شده ها
〰〰〰〰〰
🚿 *!clean rules* ===== پاک کن قوانین
💬  پاک کردن قوانین گروه
〰〰〰〰〰
🚿 *!clean silentlist* ===== پاک کن لیست سایلنت
...
if matches[1] == "پسرم" and is_mod(msg) then
text5 = [[
😍جـــــونــــم بابایـــــــی 
]]
return text5 
end
...
--------------------- Welcome -----------------------
	if matches[1] == "welcome" and is_mod(msg) or matches[1] == "خوش آمد" and is_mod(msg) then
	...
	if matches[1] == "setwelcome" and matches[2] and is_mod(msg) or matches[1] == "ایجاد خوش آمد گو" and matches[2] and is_mod(msg) then
